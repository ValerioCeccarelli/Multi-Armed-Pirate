...
from src.agent import (
    ...
    IntervalAwareBaselineAgent,
    MyBanditFeedbackPrimalDual,
    PrimalDualAgent,
)
from src.core import (
    TimeSeriesData,
    _aggregate_price_frequencies,
    _aggregate_success_failure_frequencies,
    _normalize_price_frequencies,
    _normalize_success_failure_frequencies,
    _update_price_frequencies,
    _update_success_failure_frequencies,
)
...
prices = [0.2, 0.256, 0.311, 0.367, 0.422, 0.478,
                     0.553, 0.589, 0.644, 0.7, 0.756, 0.811,
                     0.867, 0.922, 0.98, 1.001] # list(np.arange(0.1, 1, 0.1))
budget = 6_000
...
def env_builder():
    ...
    #     mean=means, std=stds, time_horizon=time_horizon
    # )
    return NonStationaryBudgetedPricingEnvironment(
        ...
        shock_prob=0.05,
        freq=50,
        num_regimes=int(time_horizon / 4)
    )
...
class NonStationaryBudgetedPricingEnvironment:
    ...
    def __init__(self, prices: Union[List[float], np.ndarray], T: int, shock_prob: float,
                 freq: int, num_regimes: int = 10000, rng: Optional[np.random.Generator] = None) -> None:
        ...
        self._valuations: np.ndarray = generate_beta_valuations(T, freq, rng=self.rng)
        self.price_set = prices

    @property
    def valuations(self) -> np.ndarray:
        return self._valuations.reshape(1, -1)

    def round(self, round: int, prices: List[float]) -> List[bool]:
        ...
        """
        valuation: float = float(self._valuations[round])
        return [valuation > prices[0]]
    ...
    def compute_sell_probabilities(self) -> np.ndarray:
        ...
        sell_probabilities: np.ndarray = np.array([
            float(np.sum(p <= self._valuations)) / self.T for p in self.price_grid
        ])
...
def agent_builder(env: Environment):
    ...
    #     beta=0.1,
    # )
    return MyBanditFeedbackPrimalDual(
        price_set=prices,
        num_items=num_items,
        B=budget,
        T=time_horizon,
...
def run_simulation(agent, env) -> RunSimulationResult:
    ...
    try:
        print(f"Starting simulation... {time_horizon} rounds")
        for t in range(time_horizon):
            ...
            _update_success_failure_frequencies(
                success_frequencies, failure_frequencies, prices_t, purchase_decisions
            )
            _update_success_failure_frequencies(
                temp_success_freq, temp_failure_freq, prices_t, purchase_decisions
            )
...
def run_multiple_simulations(
    agent_builder: Callable[[Environment], Agent],
    ...
    for trial_idx in range(num_trials):
        ...
        regret_per_round = np.array(baseline_result.agent_rewards) - np.array(
            agent_result.agent_rewards
        )
        ...
        _aggregate_price_frequencies(
            agent_aggregated_price_frequencies,
            agent_result.price_frequencies,
            num_items,
        )
        _aggregate_success_failure_frequencies(
            agent_aggregated_success_frequencies,
            agent_result.success_frequencies,
            num_items,
        )
        _aggregate_success_failure_frequencies(
            agent_aggregated_failure_frequencies,
            agent_result.failure_frequencies,
            num_items,
        )

        _aggregate_price_frequencies(
            baseline_aggregated_price_frequencies,
            baseline_result.price_frequencies,
            num_items,
        )
        _aggregate_success_failure_frequencies(
            baseline_aggregated_success_frequencies,
            baseline_result.success_frequencies,
            num_items,
        )
        _aggregate_success_failure_frequencies(
            baseline_aggregated_failure_frequencies,
            baseline_result.failure_frequencies,
            num_items,
        )

    _normalize_price_frequencies(
        agent_aggregated_price_frequencies, num_trials, num_items
    )
    _normalize_success_failure_frequencies(
        agent_aggregated_success_frequencies, num_trials, num_items
    )
    _normalize_success_failure_frequencies(
        agent_aggregated_failure_frequencies, num_trials, num_items
    )

    _normalize_price_frequencies(
        baseline_aggregated_price_frequencies, num_trials, num_items
    )
    _normalize_success_failure_frequencies(
        baseline_aggregated_success_frequencies, num_trials, num_items
    )
    _normalize_success_failure_frequencies(
        baseline_aggregated_failure_frequencies, num_trials, num_items
    )
...
results = run_multiple_simulations(
    agent_builder, baseline_builder, env_builder, num_trials
)
...
def plot_budget_evolution(
    ...
    ax.axvline(
        x=budget_depleted_round,
        color="red",
        linestyle="--",
        linewidth=2,
        label=f"Avg Depletion Round: {budget_depleted_round:.0f}",
    )
...
def plot_summary_metrics(
    ...
    plot_cumulative_rewards(
        baseline_rewards, agent_rewards, depleted_budget_rounds.mean(), rounds_per_trial
    )
    ...
    plot_budget_evolution(
        depleted_budget_rounds.mean(), agent_rewards, initial_budget=initial_budget
    )
...
def plot_histogram_per_interval(
    schedule: np.ndarray, taken: np.ndarray, num_intervals: int
):
    """
    ...
    fig, axesx = plt.subplots(
        num_intervals, num_items, figsize=(5 * num_items, 4 * num_intervals)
    )
    ...
    for interval_index in range(num_intervals):
        ...
        end_round = (
            (interval_index + 1) * rounds_per_interval
        ...
        for item_idx in range(num_items):
            ...
            current_ax.set_title(
                f"Item {item_idx + 1} - Rounds {start_round + 1} to {end_round}"
            )
            current_ax.legend()
